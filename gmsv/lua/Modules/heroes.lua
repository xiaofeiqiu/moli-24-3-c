...
function module:recruitTalked(npc, charIndex, seqno, select, data) 
  ...
  -- NOTE 60 佣兵改名窗口 
  print('测试', npc, charIndex, seqno, select, data)
  if seqno == 60 then
    if select == CONST.BUTTON_确认 then
      if data ~= nil then
        data = tostring(data)
        if data == '' then
          NLG.TalkToCli(charIndex, -1, '名称不可为空', 4, 1)
        else
          local heroData = sgModule:get(charIndex, "heroSelected");
          local heroIndex = heroData.index;
          local oriName = Char.GetData(heroIndex, CONST.CHAR_名字)
          self:renameHero(charIndex, heroData, data)
          NLG.TalkToCli(charIndex, -1, '佣兵【' .. oriName .. '】已更名为: ' .. data, 4, 1)
        end
      end
    end
  end
  ...
  if seqno == 61 then
    if CONST.BUTTON_确认 then
      ...
      if data then
        ...
        local heroIndex = heroData.index;
        local oriName = Char.GetData(heroIndex, CONST.CHAR_名字);
        local oriMetamo = Char.GetData(heroIndex, CONST.CHAR_原始图档);
        self:remetamoHero(heroIndex, data)
        NLG.TalkToCli(charIndex, -1, '佣兵【' .. oriName .. '】形象已由【' .. oriMetamo .. '】变更为: 【' .. data .. '】', 4, 1)
      else
        NLG.TalkToCli(charIndex, -1, '请输入正确的形象编号(数字)', 4, 1)
      end
    end
  end
  ...
  if seqno== 2 and data>0 then
    ...
    if #heroesData>= 32 then
      NLG.SystemMessage(charIndex,"【提示】最多雇佣32名佣兵。")
      return
    end
    -- 数据初始化
    ...
    if isOwned then
      NLG.Say(charIndex,self.shortcutNpc,"【提示】佣兵【"..toGetHeroData[2].."】已经雇佣了。",CONST.颜色_红色,4)
      return
    end
...
function module:showHeroListWindow(charIndex,page)
  ...
  -- print('showHeroListWindow', heroesData)
  ...
  print('showHeroListWindow', items)
...
function module:handleCampaign(charIndex)
  ...
  if heroData.status == 1 then
    ...
    if count >=4 then
      NLG.Say(charIndex,self.shortcutNpc,"出战的佣兵不能超过4个。",CONST.颜色_红色,4)
    else
      heroData.status=1
      -- 生成假人
...
function module:gatherHeroes(charIndex)
  ...
  for _,heroData in pairs(campHeroes) do
    ...
    if(partyNum>=5) then 
      NLG.SystemMessage(charIndex, "队伍已满。");	
    ...
    if(heroIndex>0) then
      ...
      if(invitedPartyNum>0) then
        NLG.SystemMessage(charIndex, "【"..heroesFn:getHeroName(heroData).."】已是出战状态。");
      else
        -- 第一个参数 队员，第二个参数队长
...
function module:switchItem(charIndex,data)
  ...
  if playerItemIndex>=0 then
    ...
    if r<0 then
      NLG.SystemMessage(charIndex,"佣兵背包已满，请整理物品栏。")
      return
    end
  end
...
function module:switchPet(charIndex,data)
  ...
  if heroPetIndex >=0 then
    if Char.GetEmptyPetSlot(charIndex) < 0 then
      NLG.SystemMessage(charIndex,"你的宠物栏满了。")
      return;
    end
    -- 把佣兵宠物给玩家
...
function module:showBattlePetSetPoint(charIndex,page)
  ...
  if petSlot<0 then
    NLG.Say(charIndex,self.shortcutNpc,"请先设置出战宠物。",CONST.颜色_红色,4)
    return
  end
...
function module:setHeroBattleSkill(charIndex,data)
  ...
  if aiId==nil or  aiId<0 then
    NLG.SystemMessage(charIndex,"请选择有效的AI。")
    return  
  end
  ...
  NLG.SystemMessage(charIndex,"记录完成，佣兵的战斗AI是："..name)
end
...
function module:setPetBattleSkill(charIndex,data)
  ...
  if aiId==nil or  aiId<0 then
    NLG.SystemMessage(charIndex,"请选择有效的技能。")
    return  
  end
  local aiData = _.detect(heroesAI.aiData,function(ai) return ai.id==aiId end)
  local name = aiData.name
  heroData.petBattleTech = aiId;
  NLG.SystemMessage(charIndex,"记录完成，宠物的战斗AI是："..name)
end
...
function module:showFireConfirmWindow(charIndex)
  local heroData = sgModule:get(charIndex,"heroSelected");
  local windowStr="\n\n★即将遣散佣兵：【"..heroesFn:getHeroName(heroData).."】"
  .."\n\n※遣散后此佣兵会失去所有属性；装备与宠物。"
  .."\n\n★★★★★★★确定遣散此佣兵吗？★★★★★★★"
...
function module:renameHero(charIndex, heroData, newName)
  local heroIndex = heroData.index;
  Char.SetData(heroIndex, CONST.CHAR_名字, newName)
...
function module:showRemetamoOK(charIndex, newMetamo)
  local windowStr = "@g,"..newMetamo..",1,4,4,1@　新的形象为: "..newMetamo.."";
...
function module:onLevelUpEvent(charIndex)
  ...
  if heroTplData== nil then
    NLG.SystemMessage(charIndex,"错误的佣兵。")
  end
  ...
  NLG.SystemMessage(heroData.owner,name.."等级提升，自动加点："..(heroData.autoPointing))
end
...
function module:onPetLevelUpEvent(charIndex)
  ...
  local petSlot = Char.GetData(heroIndex, CONST.CHAR_战宠);
  if petSlot<0 then
    NLG.Say(charIndex,-1,"[提示]找不到宠物。",CONST.颜色_红色,4)
    return
  end
  ...
  NLG.SystemMessage(heroData.owner,name.."的宠物【"..petName.."】等级提升，自动加点："..(heroData.petAutoPointing))
end
...
function module:setAutoPionting(charIndex,data)
  ...
  NLG.SystemMessage(charIndex,name.."变更了自动加点方案。")
  self:showHeroOperationSecWindow(charIndex)
end
...
function module:setPetAutoPionting(charIndex,data)
  ...
  local name = Char.GetData(heroData.index,CONST.CHAR_名字)
  NLG.SystemMessage(charIndex,name.."变更了宠物的自动加点方案。")
  self:showHeroOperationSecWindow(charIndex)
end
...
function module:showCrystalSelection(charIndex)
  local items={"地属性水晶","水属性水晶","火属性水晶","风属性水晶"}
  local title="选择水晶："
  local windowStr = self:NPC_buildSelectionText(title,items);
...
function module:getSkill(charIndex,data)
  ...
  local name = Char.GetData(heroData.index,CONST.CHAR_名字)
  NLG.SystemMessage(charIndex,name.."技能记录成功。")
end
...
function module:onLoad()
  ...
  -- npc 
  self.shortcutNpc = self:NPC_createNormal('佣兵介绍人', 99262, { x = 232, y = 83, mapType = 0, map = 1000, direction = 6 });
