...
local nameMap={
  ...
  equipLocation={
    ...
    [tostring(CONST.EQUIP_腿)]="「足部」",
    [tostring(CONST.EQUIP_首饰1)]="「饰品1」",
    [tostring(CONST.EQUIP_首饰2)]="「饰品2」",
    [tostring(CONST.EQUIP_水晶)]="「水晶」",
  }
}
-- NOTE 出战选项
local heroOpList=function(status) return {nameMap['status'][tostring(status)],"备战状态","遣散"} end
-- NOTE 物品的所有属性key
local itemFields = { }
for i = 0, 0x4b do
...
function module:buildAttrDescriptionForHero(heroData)
  ...
  local windowStr = "战斗状态:"..nameMap["status"][tostring(heroData.status)]
    .. "\n\n等级:"..heroData['attr'][tostring(CONST.CHAR_等级)]
    .."\n\n未加点数:"..heroData['attr'][tostring(CONST.CHAR_升级点)]
	  -- .."\n\n攻击力:"..heroData['attr'][tostring(CONST.CHAR_攻击力)]
	  -- .."\n防御力:"..heroData['attr'][tostring(CONST.CHAR_防御力)]
	  -- .."\n敏捷:"..heroData['attr'][tostring(CONST.CHAR_敏捷)]
	  -- .."\n精神:"..heroData['attr'][tostring(CONST.CHAR_精神)]
	  -- .."\n回复:"..heroData['attr'][tostring(CONST.CHAR_回复)]
	  -- .."\n\n必杀:"..heroData['attr'][tostring(CONST.CHAR_必杀)].."反击:"..heroData['attr'][tostring(CONST.CHAR_反击)]
	  -- .."\n命中:"..heroData['attr'][tostring(CONST.CHAR_命中)].."闪躲:"..heroData['attr'][tostring(CONST.CHAR_闪躲)]
	  -- .."\n\n抗中毒:"..heroData['attr'][tostring(CONST.CHAR_抗毒)].."抗昏睡:"..heroData['attr'][tostring(CONST.CHAR_抗睡)].."抗石化:"..heroData['attr'][tostring(CONST.CHAR_抗石)]
	  -- .."\n抗酒醉:"..heroData['attr'][tostring(CONST.CHAR_抗醉)].."抗混乱:"..heroData['attr'][tostring(CONST.CHAR_抗乱)].."抗遗忘:"..heroData['attr'][tostring(CONST.CHAR_抗忘)]
	  -- .."\n\n地:"..heroData['attr'][tostring(CONST.CHAR_地属性)].."水:"..heroData['attr'][tostring(CONST.CHAR_水属性)].."火:"..heroData['attr'][tostring(CONST.CHAR_火属性)].."风:"..heroData['attr'][tostring(CONST.CHAR_风属性)]
...
function module:buildDescriptionForCampHero(heroData,page)
  ...
  local Tribe = Char.GetData(heroIndex,CONST.CHAR_种族)
  ...
  local Attrib_Earth = Char.GetData(heroIndex,CONST.CHAR_地属性)
  local Attrib_Water = Char.GetData(heroIndex,CONST.CHAR_水属性)
  local Attrib_Fire = Char.GetData(heroIndex,CONST.CHAR_火属性)
  local Attrib_Wind = Char.GetData(heroIndex,CONST.CHAR_风属性)
...
function module:buildDescriptionForPet(heroData,petIndex,page)
  ...
  local Tribe = Char.GetData(petIndex,CONST.CHAR_种族)
  ...
  local Attrib_Earth = Char.GetData(petIndex,CONST.CHAR_地属性)
  local Attrib_Water = Char.GetData(petIndex,CONST.CHAR_水属性)
  local Attrib_Fire = Char.GetData(petIndex,CONST.CHAR_火属性)
  local Attrib_Wind = Char.GetData(petIndex,CONST.CHAR_风属性)
...
function module:generateHeroDummy(charIndex,heroData)
  ...
  local mapType = Char.GetData(charIndex, CONST.CHAR_地图类型);
  local targetX = 0;
  local targetY = 0;
  local targetMapId = 0;
  ...
  targetMapId = Char.GetData(charIndex,CONST.CHAR_地图);

  Char.SetData(heroIndex, CONST.CHAR_X, targetX);
  Char.SetData(heroIndex, CONST.CHAR_Y, targetY);
  Char.SetData(heroIndex, CONST.CHAR_地图, targetMapId);
  Char.SetData(heroIndex, CONST.CHAR_地图类型, mapType);
  ...
  if heroTplData== nil then
    NLG.SystemMessage(dummyIndex,"存在未知佣兵。")
  end
...
function module:buildCampHeroPetOperator(charIndex,heroData)
  ...
  local items={}
  table.insert(items,"宠物交换")
  if petIndex>=0 then
    
    if (Char.GetData(petIndex, CONST.PET_DepartureBattleStatus) == CONST.PET_STATE_战斗) then
      table.insert(items,"宠物待命")
      
    else
      table.insert(items,"宠物出战")
      
    end
    table.insert(items,"宠物状态")
...
function module:heal(charIndex,treatTarget)
  ...
  if totalCost>0 then
    if money>totalCost then
      ...
      NLG.SystemMessage(charIndex, treatTargetName.."补充了生命与魔力，扣除了"..totalCost.."魔币。");
    else
      NLG.SystemMessage(charIndex, "抱歉，您的魔币不足，"..treatTargetName.."需要"..totalCost.."魔币才能补充生命与魔力。");
    end
  ...
  if (injured < 1) then
    NLG.SystemMessage(charIndex, treatTargetName.."无需治疗。");
  else
    for k,v in pairs(healPrice) do
      if injured>= v[1] and injured<=v[2] then
        if money>= v[3] then
          ...
          NLG.SystemMessage(charIndex, treatTargetName.."治疗完毕，扣除了"..v[3].."魔币。");
        else
          NLG.SystemMessage(charIndex, "抱歉，您的魔币不足以支付"..treatTargetName.."治疗需要的"..v[3].."魔币。");
          return 
        end
      end
    end
  ...
  if money >= cost and soulLost > 0 then
    ...
    -- NLG.UpChar(treatTarget);
    NLG.SystemMessage(charIndex,"招魂完成，扣除了"..cost.."魔币。");	
  
  end
  if money < cost then
    NLG.SystemMessage(charIndex,"缺少招魂所需的"..cost.."魔币。");	
  end
...
function module:rename(charIndex, heroData, page)
  ...
  local windowStr="　【" .. heroTplData[15] .. "】　" .. oriName .."　　　职业：" .. jobName .. "\n\n　\n　请输入新的名字：\n\n"
...
function module:setPoint(charIndex,heroIndex)
  ...
  if restPoint<pointsBeSetted then
    NLG.SystemMessage(charIndex, "点数分配错误，请按照提示填写。");
    return
  end
  -- 判断 分配点数是否超过总点数的一半
  local vital = Char.GetData(heroIndex,CONST.CHAR_体力)/100
  ...
  for key,arr in pairs(pointAttrs) do
    ...
    if data+originData>totalPoint/2 then
      ...
      NLG.Say(charIndex,-1,"点数分配错误，单项点数超过最大值，一般为了总点数的一半。",CONST.颜色_红色,0)
      return 
    end
    ::continue::
  end
...
function module:buildHeroOperationSecWindow(charIndex,heroData)
  ...
  local items = {"佣兵加点","宠物加点","佣兵自动加点方案("..(heroData.autoPointing or '未选择')..")","宠物自动加点方案("..(heroData.petAutoPointing  or '未选择')..")","佣兵自动加点开关【"..labelAutoPointing.."】","宠物自动加点开关【"..labelPetAUtoPointing.."】"}
  return self:NPC_buildSelectionText(title,items);
...
function module:buildAutoPointSelect(type)
  ...
  local title="★选择加点方案(体力/力量/强度/敏捷/魔法)"
...
function module:deleteHeroData(charIndex,heroData)
  ...
  NLG.SystemMessage(charIndex, heroData.name.."已遣散这位佣兵，有缘再见吧。");
end
