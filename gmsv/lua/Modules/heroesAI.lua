...
module.conditions = {
	...
	["36"] = {
		comment = "对方某一单位mp>50且<75%",
		fn = function(charIndex, side, battleIndex)
			local defChars = getDeffenderSide(charIndex, side, battleIndex)
			return _.any(defChars, function(charIndex)
				local mp = Char.GetData(charIndex, CONST.CHAR_魔)
				local mpRatio = Char.GetData(charIndex, CONST.CHAR_魔) / Char.GetData(charIndex, CONST.CHAR_最大魔)
...
function module:showChooseType(charIndex, data, heroData)
	...
	local items = { "设置佣兵AI", "设置宠物AI", }

	local title = "★请选择要设置AI对象："
...
function module:getAI(charIndex, data)
	...
	NLG.SystemMessage(charIndex, name .. " 记录了此AI，请在佣兵右键菜单启用。")
end
...
function module:onLoad()
	...
	-- 秋兔的
	self.AINpc = self:NPC_createNormal('佣兵战术', 99262, { x = 233, y = 83, mapType = 0, map = 1000, direction = 6 });
