...
local itemList = {
  { name = '血池补充（10000LP）', image = 27243, price = 1000, desc = '补充血池使用量10000点', count = 1, maxCount = 999, value = 10000, type = 'lp' },
  { name = '血池补充（100000LP）', image = 27243, price = 10000, desc = '补充血池使用量100000点', count = 1, maxCount = 999, value = 100000, type = 'lp' },
  { name = '血池补充（1000000LP）', image = 27243, price = 100000, desc = '补充血池使用量1000000点', count = 1, maxCount = 999, value = 1000000, type = 'lp' },
  { name = '魔池补充（10000FP）', image = 26206, price = 1000, desc = '补充魔池使用量10000点', count = 1, maxCount = 999, value = 10000, type = 'fp' },
  { name = '魔池补充（100000FP）', image = 26206, price = 10000, desc = '补充魔池使用量100000点', count = 1, maxCount = 999, value = 100000, type = 'fp' },
  { name = '魔池补充（1000000FP）', image = 26206, price = 100000, desc = '补充魔池使用量1000000点', count = 1, maxCount = 999, value = 1000000, type = 'fp' },
...
function Module:onLoad()
  self:logInfo('load')
  local npc = self:NPC_createNormal('血魔池', 99262, { x = 234, y = 84, mapType = 0, map = 1000, direction = 6 })
...
function Module:heal(charIndex,targetIndex)
  ...
  if lpPool <= 0 and fpPool <= 0 then
    NLG.SystemMessage(charIndex, '[血魔池] 剩余容量不足，请及时补充。');
    return
  end
  ...
  if lpPool > 0 and lp < maxLp then
    ...
    NLG.SystemMessage(charIndex, '[血魔池] 已为'..name..'恢复: ' .. (maxLp - lp) .. 'LP, 血池剩余: ' .. lpPool);
  else
    maxLp = lp;
  end
  ...
  if fpPool > 0 and fp < maxFp then
    ...
    NLG.SystemMessage(charIndex, '[血魔池] 已为'..name..'恢复: ' .. (maxFp - fp) .. 'FP, 魔池剩余: ' .. fpPool);
  else
    maxFp = fp;
  end
  ...
  if petSlot >= 0 then
    ...
    if lpPool > 0 and lp < maxLp then
      ...
      NLG.SystemMessage(charIndex, '[血魔池] 已恢复宠物: ' .. (maxLp - lp) .. 'LP, 血池剩余: ' .. lpPool);
    else
      maxLp = lp;
    end
    ...
    if fpPool > 0 and fp < maxFp then
      ...
      NLG.SystemMessage(charIndex, '[血魔池] 已恢复宠物: ' .. (maxFp - fp) .. 'FP, 魔池剩余: ' .. fpPool);
    else
      maxFp = fp;
    end
...
function Module:onSellerTalked(npc, player)
  if NLG.CanTalk(npc, player) then
    local npcInfo = self:NPC_buildBuyWindowData(101024, '血魔池补充', '充值血魔池', '金钱不足', '背包已满', itemList);
